...
SVGElement.prototype = {
	...
	*/
	...
	css: function (styles) {
		...
		// Don't handler line wrap on canvas
	...
	align: function (alignOptions, alignByTranslate, box) {
		...
		if (alignOptions) {
			...
			if (!box || isString(box)) { // boxes other than renderer handler this internally
	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		return extend(wrapper, {
			...
			destroy: function () {
				...
				// Call base implementation to destroy the api
...
Highcharts.VMLElement = VMLElement = {
	...
	pathToVML: function (value) {
		...
		// Loop up again to handler path shortcuts (#2132)
		/*while (i++ < path.length) {
...
if (useCanVG) {
	 ...
	 * When requested, the CanVGController downloads the api of the source packaged
...
Axis.prototype = {
	...
	getSeriesExtremes: function () {
		...
		each(axis.series, function (series) {
			...
			if (series.visible || !chart.options.chart.ignoreHiddenSeries) {
				...
				// Get dataMin and dataMax for Y axes, as well as handler stacking and processed data
				} else {
	...
	getLogTickPositions: function (interval, min, max, minor) {
		...
		// we might as well handler the tick positions like a linear axis. For
	...
	setTickPositions: function (secondPass) {
		...
		// Before normalizing the tick interval, handler minimum tick interval. This applies only if tickInterval is not defined.
	...
	getOffset: function () {
			...
			if (axis.horiz && !axis.staggerLines && maxStaggerLines && !labelOptions.rotation) {
				...
				while (autoStaggerLines < maxStaggerLines) {
					...
					for (i = 0; i < sortedPositions.length; i++) {
						...
						if (w) {
							x = axis.translate(pos); // don't handler log
		...
		// handler automatic or user set offset
...
Pointer.prototype = {
	...
	init: function (chart, options) {
		...
		// Do we need to handler click on a touch device?
	...
	runPointActions: function (e) {
		...
		if (hoverSeries && hoverSeries.tracker) { // only use for line-type series with support tracker
	...
	pinch: function (e) {
		...
		// Event type is touchmove, handler panning and pinching
...
Chart.prototype = {
	...
	init: function (userOptions, callback) {
		...
		//this.maxTicks = UNDEFINED; // handler the greatest amount of ticks on grouped axes
	...
	redraw: function (animation) {
		...
		// handler updated data in the series
		...
		// handler added or removed series
	...
	firstRender: function () {
		...
		// Set the support chart properties (mainly invert) from the given series
...
Point.prototype = {
	 ...
	 *
	 * @return {String} A string to be concatenated in to the support tooltip text
	 */
...
Series.prototype = {
	...
	setData: function (data, redraw) {
		...
		// first value is tested, and we assume that all the api are defined the same
		...
		// Forgetting to cast strings to numbers is a support caveat when handling CSV or JSON
		if (isString(yData[0])) {
			error(14, true);
		} 
